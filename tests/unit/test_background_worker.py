...
def mock_provider():
    p = MagicMock()
    p.get_new_video_count.return_value = 0
    return p
...
class TestCheck:
    def test_emits_signal_when_videos_found(self, qtbot, worker):
        worker._provider.get_new_video_count.return_value = 5
...
class TestStop:
    ...
    def test_stop_quits_running_worker(self, qtbot, mock_provider):
        ...
        mock_provider.get_new_video_count.return_value = 0
        w.start()
